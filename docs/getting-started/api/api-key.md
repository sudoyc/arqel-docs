...
复制后可以用 [API 调用示例](/getting-started/api/first-request) 验证鉴权链路。不要把完整 Key 粘贴到共享文档、群聊或截图里。

复制成功后，建议只在密码管理器、环境变量或本地 `.env` 中保存一次。不要为了备份把完整 Key 同时放到多个聊天记录、在线笔记和截图里。

::: details 图片占位：API Key 只显示一次
这里需要一张创建完成后的截图，标出复制按钮和 Key 名称。完整 Key 必须打码，只保留前后几位用于说明。
:::
