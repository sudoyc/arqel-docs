...
如果控制台提供 `Use Key`、SDK 示例或 cURL 示例，优先从这些配置片段里复制 Base URL。不要复制浏览器地址栏里的控制台网页地址。

![Base URL 定位：从 Use Key 配置片段复制根地址](/docs-media/api/base-url-locator.webp)

上图来自 Arqel 控制台真实 `Use Key` 配置片段。复制 Base URL 时看配置片段或控制台明确标出的 API 根地址，不要复制浏览器地址栏。
...
| 名称 | 示例 | 用途 |
| --- | --- | --- |
| Base URL | `https://arqel.net` | API 根地址，填到 SDK 或工具的 Base URL 字段 |
| 请求路径 | `/chat/completions` | 聊天补全接口路径 |
| 完整请求地址 | `https://arqel.net/chat/completions` | cURL 手动请求时会用到 |

常见错误是把 `/chat/completions` 也复制进 Base URL，或者把完整请求地址填进只需要 Base URL 的工具字段里。

如果找不到 Base URL，按这个顺序排查：

1. 回到 Arqel 控制台，不要在文档站或浏览器地址栏里复制。
2. 查找 `API`、`Endpoint`、`Base URL`、`Provider`、`开发者设置`、`请求示例`。
3. 如果页面提供 cURL 示例，先看 `https://.../chat/completions` 这一类完整地址，再把 `/chat/completions` 前面的部分作为 Base URL。
4. 如果控制台给出的地址和本文示例不同，以控制台为准。

成功状态应该是：你能明确得到一个以 `https://` 开头的 API 根地址。

## 到哪里找模型名

请在 Arqel 控制台的模型列表、模型管理或请求示例区域查找可用模型名。最稳妥的方式是复制控制台请求示例里的 `model` 字段。

![模型名定位：从控制台复制真实模型字符串](/docs-media/api/model-name-locator.webp)
