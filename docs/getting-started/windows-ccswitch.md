...
| Windows 用户，想尽快接入 Codex / Claude Code / Gemini CLI | 优先用 CC Switch |
...
4. 你要使用的工具，例如 Codex、Claude Code 或 Gemini CLI。
...
在 CC Switch 顶部应用切换器里，先选择你要接入的工具，例如 Codex。切换后，Provider 列表会显示当前工具的 Provider。

应用专属 Provider 只作用于当前选中的工具。不要在 Claude 页面添加完 Provider 后，直接假设 Codex 也会使用它。

## 4. 在当前工具页面添加 Arqel Provider

在当前工具页面添加一个 Provider：
...
保存后，启用这个 Provider。先不要一次性切到所有工具。先选一个你最常用的工具验证。

更细的字段说明见 [按应用添加 Arqel Provider](/tools/cc-switch/provider)。

## 5. 按目标工具验证

| 工具 | 在 CC Switch 里的操作 | 验证方式 |
| --- | --- | --- |
| Codex | 选择 Codex，添加并启用 Arqel Provider | 重启 Codex App / CLI / IDE Extension，执行只读任务 |
| Claude Code | 选择 Claude Code，添加并启用 Arqel Provider | 重启 Claude Code，问一个只读问题 |
| Gemini CLI | 选择 Gemini CLI，添加并启用 Arqel Provider | 重开终端，运行 Gemini CLI，问一个简单问题 |
| Hermes Agent | 配置前先确认当前 Hermes 官方配置 | 只做只读测试 |
...
Codex 的本地 App、CLI、IDE Extension 共享同一套 Codex 配置。用 CC Switch 时，先选择 Codex，再添加并启用 Arqel Provider；之后重启你实际使用的 Codex 入口并验证。Codex Web / cloud 和云端集成不在这条本地配置路径里，需要单独确认。
...
- [按应用添加 Arqel Provider](/tools/cc-switch/provider)
