...
如果你是 Windows 新手，直接看 [新手入门：Codex + CC Switch](/getting-started/agent-quickstart)。
...
- **新手入门**：给 Windows 新手用户，从准备 Codex 入口，到用 CC Switch 确认请求记录。
...
4. 如果 CC Switch 不适合，再按工具页的 macOS / Windows / Linux 手动配置说明处理。
5. 用只读问题验证接入。
