...
这页给完全不熟悉开发工具的 Windows 用户，解决"怎么用 Arqel"的问题。你只需要完成一条路径：准备 Codex 入口，安装 CC Switch，准备 Arqel 接入三要素，在 CC Switch 里选择 Codex，再把 Arqel Provider 添加进去，最后用 Codex 做一次只读测试，回到 Arqel 控制台确认请求记录。

本页默认从 **Codex** 入门。Codex App、Codex CLI、Codex IDE Extension 使用同一套本地 Codex Provider 配置；你可以把 Codex App 当作日常入口。文中仍保留 Codex CLI 命令，是因为命令行结果更容易复现和排查。
...
判断是否完成，不看界面长得是否和别人一样，只看上面三项状态是否成立。第三方工具界面可能改版，字段名称也可能变化；只要能确认当前页面是 Codex、Provider 是 Arqel、控制台有对应请求记录，就可以认为本页完成。

![最终效果总览：CC Switch 已启用 Arqel Provider，Arqel 控制台出现请求记录](/docs-media/getting-started/quickstart-success-overview.webp)
...
确认标准很简单：你能从开始菜单、桌面快捷方式或 IDE 插件入口打开 Codex，并进入可以输入任务的界面。此时不要急着测试 Arqel，因为 Provider 还没有写入。
...
验证时只看命令是否返回版本号，不要求你的版本号和文档示例一致。如果 `node -v` 或 `npm -v` 没有结果，先回到 Node.js 安装步骤；如果只有 `codex --version` 没有结果，再重新运行 Codex 安装命令。
...
在 GitHub Releases 页面下载时，通常要展开 `Assets` 区域，再选择名称里带有 `Windows.msi` 的安装包。不要下载 `Source code` 压缩包；那是源码，不是给新手直接安装的 Windows 安装器。

![CC Switch 下载页面中的 Windows MSI 安装包位置](/docs-media/getting-started/cc-switch-download-page.png)
...
1. 先在 API Key 页面创建或复制一个专门给 Codex 用的 Key。
2. 再找 `Use Key`、`API`、`Endpoint`、`Base URL` 或请求示例区域，复制控制台明确给出的 API 根地址。
3. 最后在模型列表或请求示例里复制完整模型名。

Base URL 不要从浏览器地址栏复制。浏览器地址栏通常是控制台网页地址，不是给 Agent 调用模型的 API 地址。

![Arqel 接入三要素：API Key、Base URL、模型名](/docs-media/getting-started/arqel-three-elements.webp)

图中的 Base URL 来自 Arqel 控制台真实 `Use Key` 配置片段。不要从浏览器地址栏复制 Base URL。

## 6. 先选择 Codex

CC Switch 是按应用管理 Provider 的。请先在 CC Switch 顶部的应用切换器里选择 **Codex**，再添加 Provider。

你要确认当前页面显示的是 Codex，而不是 Claude Code 或其他工具。只有在 Codex 页面里添加的应用专属 Provider，才会写入 Codex 的配置。

如果 CC Switch 顶部有应用名、图标、标签页或下拉框，请以那里显示的当前应用为准。切换到 Codex 后，再看 Provider 列表和新增按钮。
...
保存后请回到 Codex 的 Provider 列表确认两件事：

1. 列表里能看到你刚才创建的 `Arqel` Provider。
2. 这个 Provider 处于 Enabled、Active、Current 或类似状态。

如果界面只有“保存”没有“启用”，请查当前 CC Switch 版本是否需要在 Provider 列表里再选择一次。

![在 Codex 页面进入新增 Provider，并确认 Arqel Provider 已启用](/docs-media/getting-started/cc-switch-add-provider.webp)

这段动图展示进入新增 Provider 页面，以及回到列表确认 Arqel Provider 已启用。实际填写字段以上方表格和你当前 CC Switch 界面为准。
...
如果控制台支持按 Key、模型或时间筛选，先按刚才的测试时间缩小范围。请求记录可能有短暂延迟；刷新后仍没有记录，再按下面表格排查。

![Arqel 控制台请求记录中出现 Codex 请求](/docs-media/getting-started/arqel-request-log-check.webp)
