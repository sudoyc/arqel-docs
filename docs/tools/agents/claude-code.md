...
1. 安装并打开 CC Switch。
2. 在应用切换器里选择 Claude Code。
3. 在 Claude Code 页面新增 Arqel Provider，填写 API Key、Base URL 和 Arqel 控制台里的具体模型名。
...
这些变量仅说明 Claude Code 侧存在对应配置项，协议兼容性需按本页顶部 [协议边界](#) 说明确认。若 Arqel 控制台没有请求记录，先回到当前版本官方文档确认 Provider / Gateway 入口。
...
检查 CC Switch 是否在 Claude Code 页面启用了正确 Provider、是否重启 Claude Code、当前运行环境是否和 CC Switch 管理的环境一致。Windows 终端、WSL、IDE 插件可能不是同一套配置。
