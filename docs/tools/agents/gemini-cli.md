...
::: warning 协议兼容
OpenAI-compatible Base URL 不代表 Gemini CLI 一定可以直接使用。Gemini CLI 可能需要 Gemini 原生协议、版本特定的 Provider 配置，或经过验证的适配路径；Arqel 是否支持该路径需要以产品确认或实测为准。接入前请先阅读 [工具接入总览](/tools/) 的协议兼容说明。
:::
...
如果工具明确要求 OpenAI 风格变量，并且该 Provider 路径已确认可用于 Arqel，可以按工具说明映射：
...
::: warning
变量名和 Provider 类型以实际 Gemini CLI 版本为准。这些变量不是 Gemini 原生协议支持证明，也不代表 Arqel 已支持 Gemini-compatible 协议。如果你的版本不读取这些变量，不要继续反复改 Key，先查看 `gemini --help`、当前版本文档，或改用 CC Switch 做 Provider 管理并逐个验证。
:::
...
如果工具明确要求 OpenAI 风格变量，并且该 Provider 路径已确认可用于 Arqel，可以映射：
...
这些变量只适用于明确读取 OpenAI 风格配置的路径。若 Arqel 控制台没有请求记录，说明当前 Gemini CLI 入口可能没有走这组配置。
