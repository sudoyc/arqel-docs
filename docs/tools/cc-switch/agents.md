...
根据 CC Switch README 和 release notes，它可以管理 Claude Code、Codex、Gemini CLI、OpenCode、OpenClaw 和 Hermes Agent 的配置。managed app 不等于 Arqel 已验证对应协议。
...
CC Switch v3.14.0+ 将 Hermes Agent 作为 managed app。以下信息来自 CC Switch release context，仅用于理解 CC Switch 管理范围，不代表 Arqel 已支持这些协议：
...
使用前请确认 Hermes 官方安装方式和当前版本配置项。在 Arqel 明确验证前，请把 Hermes Agent 视为待验证路径，只做只读测试。更多说明见 [Hermes Agent](/tools/agents/hermes)。
