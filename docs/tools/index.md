...
这里说的"工具"指 Cursor、Claude Code、Codex、Hermes Agent 等具体产品入口，不是泛指普通 CLI。
...
CC Switch 适合 Codex、Claude Code 等 Agent 工具。它能减少重复填写 API Key、Base URL 和模型名，也方便切换 Provider。
...
Cursor 主要是桌面编辑器，通常先在 Cursor 自己的 Models、API、OpenAI-compatible 或 Custom API 设置里填写 Arqel 信息。不要默认认为 Cursor 会读取终端里 Claude Code 或 Codex 的配置。
...
手动配置时，请先看对应工具页。不要把 Claude Code、Codex、Cursor 的配置文件互相套用。
