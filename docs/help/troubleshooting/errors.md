...
3. 如果是 Agent，确认 Base URL 填在当前工具真正读取的 Provider / Endpoint 字段里。
4. 如果是 API / SDK，检查请求路径。
5. 重新从控制台复制模型名。
...
2. 换一个控制台中可用的具体模型名测试。
3. 如果是 Agent 工具，先确认控制台是否出现该 Agent 请求记录，以及是否只有某个 Agent 失败。
4. 只有在深度排障时，才需要用 API 调用示例或 SDK 单独检查请求结构。
5. 如果持续失败，联系支持。
